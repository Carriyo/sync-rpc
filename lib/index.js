...
function start() {
  ...
  const port = findPort();
  childProcess = spawn(process.execPath, [require.resolve('./worker'), port], {
    stdio: 'inherit',
    windowsHide: true,
  });
  childProcess.unref();
...
/**
 * This function may be needed for:
 * 1. node.js debugger to detach after main process exits.
 * 2. For stdin to close properly. Some test runner reports like tap-reader
 *    doesn't exit properly without this.
 */
function cleanup() {
  if (childProcess) {
    childProcess.kill();
